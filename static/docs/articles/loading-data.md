...
[`dataset()`](https://torch.mlverse.org/docs/reference/dataset.md) to
create a custom `Dataset` class you’ll want to define the following:
...
[`dataset()`](https://torch.mlverse.org/docs/reference/dataset.md) to
define a custom class and then loaded it in batches with a data loader.
By using the dataset and data loader we were able to write code that
split the data preprocessing and setup from the model training itself.
