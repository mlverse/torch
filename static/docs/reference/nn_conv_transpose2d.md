...
[nn_conv2d](https://torch.mlverse.org/docs/reference/nn_conv2d.md) and a
nn_conv_transpose2d are initialized with same parameters, they are
inverses of each other in regard to the input and output shapes.
However, when `stride > 1`,
[nn_conv2d](https://torch.mlverse.org/docs/reference/nn_conv2d.md) maps
multiple input shapes to the same output shape. `output_padding` is
