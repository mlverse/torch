test_that("call_torch_function works as expected", {
  ...
  expect_warning(x_grad <- call_torch_function("torch_tanh_backward", 1, y),
  ...
  expect_error(call_torch_function("torsh_tanh_backward", 1, y, quiet = TRUE), 
               "torch_")
  
  expect_error(call_torch_function("torch_tank_backward", 1, y, quiet = TRUE), 
               "check your spelling")
})
