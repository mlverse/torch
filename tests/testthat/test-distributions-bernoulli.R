...
test_that("Bernoulli distribution - basic tests", {
  ...
  expect_equal(distr_bernoulli(p)$entropy(), 
...
test_that("Bernoulli Distribution - enumerate support", {
  ...
  examples <- list(
    ...
    list(list(probs = matrix(c(0.1, 0.1, 0.3, 0.4), 2, 2)), 
...
test_that("Bernoulli Distribution 3D", {
  ...
  expect_equal(distr_bernoulli(p)$sample(sample_shape=c(2, 5))$size(),
               c(2, 5, 2, 3, 5))
  expect_equal(distr_bernoulli(p)$sample(2)$size(),
               c(2, 2, 3, 5))
})
