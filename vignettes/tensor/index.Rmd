...
If `keepdim` is `TRUE`, the output tensor is of the same size as
`input` except in the dimension :attr:`dim` where it is of size 1.
Otherwise, `dim` is squeezed (see [torch_squeeze()]), resulting
in the output tensor having 1 fewer dimension than `input`.
...
sampled from `\text{Bernoulli}(\texttt{self[i]})`. :attr:`self` must have
...
`\text{Bernoulli}(\texttt{p})`. :attr:`self` can have integral
`dtype`.
...
The `\text{i}^{th}` element of :attr:`self` tensor will be set to a
value sampled from `\text{Bernoulli}(\texttt{p\_tensor[i]})`.

`self` can have integral `dtype`, but :attr:`p_tensor` must have
...
Copies the elements from `src` into :attr:`self` tensor and returns
`self`.

The `src` tensor must be :ref:`broadcastable <broadcasting-semantics>`
...
Accumulate the elements of `tensor` into the :attr:`self` tensor by adding
to the indices in the order given in `index`. For example, if `dim == 0`
and `index[i] == j`, then the `i`\ th row of `tensor` is added to the
`j`\ th row of `self`.

The `dim`\ th dimension of :attr:`tensor` must have the same size as the
...
Copies the elements of `tensor` into the :attr:`self` tensor by selecting
...
Fills the elements of the `self` tensor with value :attr:`val` by
...
Puts values from the tensor `value` into the tensor :attr:`self` using
...
If `accumulate` is `TRUE`, the elements in :attr:`value` are added to
...
`\sigma`. Note that :attr:`mean` and :attr:`std` are the mean and
...
Fills elements of `self` tensor with :attr:`value` where :attr:`mask` is
...
Copies elements from `source` into :attr:`self` tensor at positions where
the `mask` is TRUE.
...
The `mask` operates on the :attr:`self` tensor, not on the given
`source` tensor.
...
Returns a Tensor of size `size` filled with :attr:`fill_value`.
By default, the returned Tensor has the same `torch_dtype` and
`torch_device` as this tensor.
...
parameterized by `mean` and :attr:`std`.
...
If `accumulate` is `TRUE`, the elements in :attr:`tensor` are added to
`self`. If accumulate is `FALSE`, the behavior is undefined if indices
contain duplicate elements.
...
Refines the dimension names of `self` according to :attr:`names`.
...
Writes all values from the tensor `src` into :attr:`self` at the indices
specified in the `index` tensor. For each value in :attr:`src`, its output
...
Adds all values from the tensor `other` into :attr:`self` at the indices
specified in the `index` tensor in a similar fashion as
`~$scatter_`. For each value in :attr:`src`, it is added to
an index in `self` which is specified by its index in :attr:`src`
...
`self`, :attr:`index` and :attr:`src` should have same number of
...
by indices of `mask` and values are ignored. :attr:`input` and :attr:`mask`
...
Subtracts a scalar or tensor from `self` tensor. If both :attr:`alpha`
and `other` are specified, each element of :attr:`other` is scaled by
`alpha` before being used.

When `other` is a tensor, the shape of :attr:`other` must be
`broadcastable <broadcasting-semantics>` with the shape of the underlying
tensor.
...
`dtype`. If :attr:`dtype` is `NULL` it is inferred to be `self$dtype`.
...
the Tensor `other`. When :attr:`non_blocking`, tries to convert
...
`self` tensor in the dimension :attr:`dimension`.

Step between two slices is given by `step`.

If `sizedim` is the size of dimension `dimension` for :attr:`self`, the size of
...
Otherwise, it will not be possible to view `self` tensor as :attr:`shape`
