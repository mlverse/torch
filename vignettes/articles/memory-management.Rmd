...
reassigned, the old tensor's memory is returned to the system allocator right away.
R uses garbage collection (GC) instead, meaning old tensors accumulate in memory
until GC runs. For large tensors (roughly > 1 MB), the system allocator uses
`mmap`/`munmap` system calls to obtain and release memory. When many dead-but-not-yet-collected
tensors are still mapped, new allocations can't reuse that memory and must request
fresh pages from the OS — which is expensive.

### CPU block cache

To compensate, torch includes a **CPU block cache** that intercepts freed tensor
memory and holds it in an in-process pool, keyed by allocation size. When a new
tensor of the same size is needed, the cache returns a block instantly — skipping
the costly `mmap`/`munmap` round-trip entirely. This is especially effective during
training loops, where the same tensor shapes are allocated and freed repeatedly
across iterations.

The cache is flushed automatically whenever the allocator triggers R's garbage
collector (see below), so memory is eventually returned to the OS. You can also
flush it manually with [cpu_cache_flush()].

### GC triggering

On the CPU, torch will call the R garbage collector in two situations:

1. **Periodic threshold**: Every 4 GB of cumulative allocations, the allocator
   triggers `gc()` to clean up dangling tensors. After GC runs, the block cache
   is flushed so that freed memory is returned to the OS. The threshold can be
   ...
   This option must be set before calling `library(torch)`, as it is applied at
   startup.

2. **On allocation failure**: If `alloc_cpu()` fails, the allocator first flushes
   the block cache, then retries. If that still fails, it triggers a full GC and
   retries once more.

   Note: on most UNIX-based systems (Linux, macOS), memory allocations almost
   never fail — the OS will overcommit memory and use swap instead. If swap is
   exhausted, the system may hang or the OOM killer may terminate the process,
   rather than returning an allocation error that torch could handle gracefully.

   If your R session is hanging and you believe it should have enough memory,
   try lowering `torch.threshold_call_gc` to trigger GC more frequently. This
   adds overhead but ensures tensors are released sooner.
...
CUDA memory snapshots, similar to the [PyTorch Python implementation](https://docs.pytorch.org/docs/stable/torch_cuda_memory.html).
