...
knitr::opts_chunk$set(
  collapse = TRUE,
  comment = "#>",
  eval = identical(Sys.getenv("TORCH_TEST", unset = "0"), "1") && cuda_is_available(),
  purl = FALSE
)
```
...
1. Allow the model to use different implementations of operations so it can use
   half-precision.
2. Using loss scaling to preserver small gradient values that might be too small
...
run <- function(autocast, scale) {
  ...
  for (epoch in seq_len(epochs)) {
    for (i in seq_along(data)) {
      with_autocast(enabled = autocast, {
        output <- net(data[[i]])
        loss <- loss_fn(output, targets[[i]])  
      })
