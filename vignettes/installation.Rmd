...
torch has very specific requirements in terms of CUDA and CUDNN versions it supports. We recomment installing torch
using pre-built binaries. See (#pre-built) for more information.
...
-   Have installed cuDNN - a version compatible with CUDA v12.8. Follow the installation instructions available [here](https://developer.nvidia.com/cudnn).
...
torch has very specific requirements in terms of CUDA and CUDNN versions it supports. We recommand installing torch
using pre-built binaries. See (#pre-built) for more information.
...
-   You have correctly installed the NVIDIA CUDA Toolkit versions 12.6 or 12.8, follow the instructions [here](https://docs.nvidia.com/cuda/archive/12.8.0/).
...
## Using cudatoolkit R packages {#cudatoolkit}

Instead of installing the full CUDA toolkit on your system, you can use the
`cuda12.6` or `cuda12.8` R packages to provide the necessary CUDA runtime
libraries. These packages bundle the CUDA shared libraries so torch can use
GPU acceleration without a system-wide CUDA installation.
...
When torch starts, it automatically detects installed cudatoolkit R packages
and uses them to provide CUDA libraries. If multiple versions are installed,
the newest supported version is preferred.

### Environment variables

The `TORCH_CUDATOOLKIT` environment variable controls cudatoolkit R package
detection:

- **Not set** (default): torch auto-detects installed cudatoolkit R packages.
- **Set to a version** (e.g. `TORCH_CUDATOOLKIT=12.8`): forces torch to use
  that specific cudatoolkit R package. Errors if the package is not installed.
  If `CUDA` is also set, the two values must agree.
- **Set to `FALSE`**: disables cudatoolkit R package detection entirely.
  torch will only use system CUDA (via `CUDA_HOME`, `CUDA_PATH`, or `nvcc`).

The full CUDA version detection priority is:

1. `TORCH_CUDATOOLKIT` — explicit cudatoolkit R package selection.
2. `CUDA` — if a matching cudatoolkit R package is installed, use it; otherwise
   use the value directly for system CUDA.
3. Auto-detect installed cudatoolkit R packages.
4. System CUDA — `CUDA_HOME` / `CUDA_PATH`, conventional paths, `nvcc` on PATH.

When a cudatoolkit R package is found (via any of steps 1-3), system CUDA
...
As there are many installation failure scenario, you may want to activate verbose messages for better understanding what is going on during the installation process. This is done through the `TORCH_INSTALL_DEBUG` environment variable:
