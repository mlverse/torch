...
```{r}
x1 <- torch_ones(c(2,2), options = list(requires_grad = TRUE))
...
out.grad_fn.next_functions[0][0].next_functions[0][0].next_functions[0][0].next_functions
# here we arrive at the other leaf node (AccumulateGrad for x2)
out.grad_fn.next_functions[0][0].next_functions[0][0].next_functions[0][0].next_functions[1][0].next_functions
```
...
w1 <- torch_randn(c(d_in, d_hidden), options = list(requires_grad = TRUE))
# weights connecting hidden to output layer
w2 <- torch_randn(c(d_hidden, d_out), options = list(requires_grad = TRUE))
# hidden layer bias
b1 <- torch_zeros(c(1, d_hidden), options = list(requires_grad = TRUE))
# output layer bias
b2 <- torch_zeros(c(1, d_out), options = list(requires_grad = TRUE))
### network parameters ---------------------------------------------------------
learning_rate <- 1e-4
### training loop --------------------------------------------------------------
for (t in 1:200) {
